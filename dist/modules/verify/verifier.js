 ...
 */
function splitOnConjunctions(claim) {
    let result = [claim];
...
export function compareNumbers(claim, source) {
    const claimNumbers = extractNumbers(claim);
    const sourceNumbers = extractNumbers(source);
    const matched = [];
    const claimOnly = [];
    const sourceOnly = new Set(sourceNumbers);
 ...
 */
export function verifyClaimAgainstSource(claim, sources) {
    const claimEntities = extractEntities(claim);
    const claimNumbers = extractNumbers(claim);
    let totalEntityMatches = 0;
    let totalSourceEntities = 0;
    let numberMatch = true;
    const allSourceEntities = new Set();
    let numberComparison = { matched: [], claimOnly: [], sourceOnly: [] };
    ...
    return {
        ...
        details: {
            ...
            matchedEntities: claimEntities.filter(e => Array.from(allSourceEntities).some(se => se.toLowerCase() === e.toLowerCase())),
            numberComparison,
        },
    };
}
 ...
 */
export function computeTrustScore(agentOutput, sources) {
